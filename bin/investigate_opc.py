...
def parse_arguments():
    ...
    parser.add_argument('--outdir', type=str, default="/space/grp/rschwartz/rschwartz/eval-references/OPC-investigate", help='Output directory for results')
...
def rank_and_plot(ref_subset, reference, outdir, embedding_key="scvi", n_genes=25):
    ...
    if reference == "rest":
        fname = "rest_vs_opc_oligodendrocyte_de_genes.png"
    else:
        fname = f"{reference.lower()}_vs_{'opc' if reference=='Oligodendrocyte' else 'oligodendrocyte'}_de_genes.png"
...
def main():
    ...
    # restrict to .obs["subclass"] in ["OPC", "Oligodendrocyte"]
    ref_subset = ref_subset[ref_subset.obs["subclass"].isin(["OPC", "Oligodendrocyte"])]
    ...
    ref_subset.obs[["author_cell_type", "subclass", "class", "family", "global", "dataset_title", "collection_name"]].value_counts().reset_index().to_csv(
        os.path.join(outdir, "opc_oligodendrocyte_cell_types.tsv"), sep="\t", index=False
    )
    embedding_key = "scvi"
    ...
    sc.pl.umap(
        ...
        ncols=1,
        save="_opc_oligodendrocyte.png",
    )
    # Differential expression and plotting for each reference
    rank_and_plot(ref_subset, "OPC", outdir, embedding_key)
    rank_and_plot(ref_subset, "Oligodendrocyte", outdir, embedding_key)
